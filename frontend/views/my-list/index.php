...
<section class="menu-toggle">
    <ul class="menu-toggle__list">
        <li class="menu-toggle__item menu-toggle__item--completed <?= $current === 'completed' ? $current_class : '' ?>">
            <div class="menu-toggle__svg-wrapper">
                <svg width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
                    <path fill-rule="evenodd" clip-rule="evenodd" fill="<?=$current === 'completed' ? 'white' : 'black'?>"
            ...
            <a href="<?=Url::to(['/my-list/completed'])?>">
                Завершённые
            </a>
        </li>
        <li class="menu-toggle__item menu-toggle__item--new <?= $current === 'new' ? $current_class : '' ?>">
            <div class="menu-toggle__svg-wrapper">
                <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                    <rect opacity="0.01" x="24" y="24" width="24" height="24" transform="rotate(180 24 24)" fill="<?=$current === 'new' ? 'white' : 'black'?>"/>
                    <path
                        d="M12 6C12.5523 6 13 5.55228 13 5V3C13 2.44772 12.5523 2 12 2C11.4477 2 11 2.44772 11 3V5C11 5.55228 11.4477 6 12 6Z"
                        fill="<?=$current === 'new' ? 'white' : 'black'?>"/>
                    <path
            ...
            <a href="<?=Url::to(['/my-list/new'])?>">
                Новые
            </a>
        </li>
        <li class="menu-toggle__item menu-toggle__item--active <?= $current === 'active' ? $current_class : '' ?>">
            <div class="menu-toggle__svg-wrapper">
                <svg width="21" height="19" viewBox="0 0 21 19" fill="none" xmlns="http://www.w3.org/2000/svg">
                    <path fill-rule="evenodd" clip-rule="evenodd"
                          ...
                          fill="<?=$current === 'active' ? 'white' : 'black'?>"/>
                </svg>
            </div>
            <a href="<?=Url::to(['/my-list/active'])?>">
                Активные
            </a>
        </li>
        <li class="menu-toggle__item menu-toggle__item--canceled <?= $current === 'canceled' ? $current_class : '' ?>">
            <div class="menu-toggle__svg-wrapper">
                <svg width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
                    <path fill-rule="evenodd" clip-rule="evenodd"
                          ...
                          fill="<?=$current === 'canceled' ? 'white' : 'black'?>"/>
                </svg>
            </div>
            <a href="<?=Url::to(['/my-list/canceled'])?>">
                Отменённые
            </a>
        </li>
        <li class="menu-toggle__item menu-toggle__item--hidden <?= $current === 'overdue' ? $current_class : '' ?>">
            <div class="menu-toggle__svg-wrapper">
                <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                    <rect opacity="0.01" x="24" width="24" height="24" transform="rotate(90 24 0)" fill="<?=$current === 'overdue' ? 'white' : 'black'?>"/>
                    <path fill-rule="evenodd" clip-rule="evenodd"
                          ...
                          fill="<?=$current === 'overdue' ? 'white' : 'black'?>"/>
                </svg>
            </div>
            <a href="<?=Url::to(['/my-list/overdue'])?>">
                Скрытые
            </a>
        </li>
    </ul>
        ...
        <?php if (count($tasks) > 0) : ?>
            <?php foreach ($tasks as $task) : ?>
                <div class="new-task__card">
                    ...
                    <div class="task-status <?=$task->status?>-status"><?=\taskforce\models\Task::$status_map[$task->status]?></div>
                    ...
                    <div class="feedback-card__top ">
                        ...
                        <div class="feedback-card__top--name my-list__bottom">
                            ...
                            <?php $new_messages_count = $task->getNewMessages($user_id)->count(); ?>
                            <a href="<?=Url::to(['/tasks/view', 'id' => $task->id])?>" class="my-list__bottom-chat <?= $new_messages_count ? 'my-list__bottom-chat--new' : '' ?>">
                                <?= $new_messages_count ? "<b>{$new_messages_count}</b>" : ''?>
                            </a>
                            <?= RatingWidget::widget(['rating' => $task->customer->workerRating]) ?>
                        </div>
                    </div>
                </div>
            <?php endforeach; ?>
